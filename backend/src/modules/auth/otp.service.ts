...
export class OtpService {
  ...
  private key(phone: string, ctx: string) { return `otp:${ctx}:${phone}`; }
  private genCode(): string {
    const max = Math.pow(10, this.length);
    return Math.floor(Math.random() * max).toString().padStart(this.length, '0');
  }
  ...
  async sendOtp(phone: string, ctx: 'register'|'reset'='register') {
    const code = this.genCode(); const hashed = this.hash(code);
