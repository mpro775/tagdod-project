...
export class VariantsService {
  ...
  async createVariant(dto: Partial<Variant>) {
    ...
    const enrichedAttributes = await Promise.all(
      (dto.attributeValues || []).map(async (av: any) => {
        const attr = await this.attributesService.getAttribute(av.attributeId);
        const value = attr.values?.find((v: any) => String(v._id) === String(av.valueId));
  ...
  async generateVariants(productId: string, defaultPrice: number, defaultStock: number, overwrite = false) {
  ...
  private generateCombinations(attributesWithValues: any[]): any[] {
    if (attributesWithValues.length === 0) return [];
    if (attributesWithValues.length === 1) {
      return attributesWithValues[0].values.map((v: any) => [
        {
