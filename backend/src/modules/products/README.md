...
- ✅ إضافة تسعير افتراضي (`basePriceUSD`) للمنتجات البسيطة (بدون Variants) لاستخدامه في السلة والطلب

### **🆕 ميزة المنتجات الشبيهة (Related Products):**
- ✅ إضافة حقل `relatedProducts` في Product Schema
- ✅ إدارة كاملة للمنتجات الشبيهة من لوحة التحكم
- ✅ API endpoints للأدمن (إضافة/حذف/تحديث)
- ✅ Public API endpoint للتطبيق
- ✅ Cache تلقائي (10 دقائق)
- ✅ فلترة تلقائية (منتجات نشطة فقط)

📖 **للمزيد من التفاصيل:** راجع [RELATED_PRODUCTS_FEATURE.md](./RELATED_PRODUCTS_FEATURE.md)
...
- `ts-node scripts/backfill-product-timestamps.ts [--dry-run] [--batch-size=500]`
  - يعيد تعبئة الحقول `createdAt` و `updatedAt` المفقودة في مجموعة المنتجات.
  - يستخدم الطابع الزمني المشتق من `ObjectId` كقيمة افتراضية منطقية.
  - الخيار `--dry-run` يتيح معاينة عمليات التحديث بدون تطبيقها.
  - يمكن تعديل حجم الدُفعة باستخدام `--batch-size` (القيمة الافتراضية 500).

> 💡 تأكد من تفعيل الاتصال بقاعدة البيانات (ENV) قبل تشغيل السكربت.
