...
## 🔄 حالات الطلبات (OrderStatus) - مبسط
...
## 💰 طرق الدفع (PaymentMethod) - مبسط

- `COD` - الدفع عند الاستلام
- `BANK_TRANSFER` - تحويل بنكي محلي

## 🔧 State Machine - مبسط

نظام إدارة حالات مبسط يضمن:
...
**ملاحظة مهمة:** تم توحيد `OrderStateMachine` في `utils/order-state-machine.ts` فقط. استخدم دائماً:
```typescript
import { OrderStateMachine } from '../utils/order-state-machine';
```
...
### إصدار 2.0.0 (الحالي):
- ✅ تبسيط حالات الطلبات (10 حالات بدلاً من 15)
- ✅ تبسيط طرق الدفع (COD و BANK_TRANSFER فقط)
- ✅ توحيد OrderStateMachine في utils فقط
- ✅ إصلاح حساب الخصومات (منع الحساب المزدوج)
- ✅ ربط السلة بالطلب (تحديث تلقائي إلى CONVERTED)
- ✅ إصلاح تحويل العملات (تمرير totalDiscount الكامل)
- ✅ فصل واضح بين حالات الطلب وحالات الدفع
