  ...
  async handleShippingWebhook(
    ...
    try {
      ...
      if (!trackingNumber || !status) {
        return {
          success: false,
          message: 'رقم التتبع والحالة مطلوبان',
      ...
      this.logger.error('Error processing shipping webhook:', error);
      return {
        success: false,
        message: 'فشل في معالجة إشعار الشحن',
        error: error instanceof Error ? error.message : 'Unknown error',
      };
    }
  }
  ...
  async handleInventoryWebhook(
    ...
    try {
      ...
      if (!variantId || change === undefined) {
        return {
          success: false,
          message: 'معرف المتغير والتغيير مطلوبان',
          error: 'Missing required fields',
        };
      }
      ...
      this.logger.error('Error processing inventory webhook:', error);
      return {
        success: false,
        message: 'فشل في معالجة إشعار المخزون',
        error: error instanceof Error ? error.message : 'Unknown error',
      };
    }
  }
