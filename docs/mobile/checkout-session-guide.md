...
### تدفق الشاشة المقترح
- قبل فتح شاشة الدفع:
  1. تأكد من مزامنة السلة المحلية مع الخادم عبر `POST /cart/sync` (يتم تنفيذها عند الضغط على زر «متابعة للشراء» في الواجهة).
  2. بعد نجاح المزامنة، انتقل إلى شاشة الدفع.
- عند فتح شاشة الدفع:
  1. استدعِ `POST /orders/checkout/session` مرّة واحدة بالعملة المستخدمة حاليًا.
...
- عند إضافة/إزالة كوبون:
  ...
  4. لا تعاود طلب بقية البيانات ما لم تتغير (العناوين، طرق الدفع، الأهلية)، واحتفظ بنتيجة `session` الأصلية كما هي.
...
- [ ] التحقق من أن مسار «متابعة للشراء» ينفّذ `POST /cart/sync` قبل استدعاء جلسة الدفع.
