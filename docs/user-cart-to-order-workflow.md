...
- عند كل خطوة يمكن للمستخدم إعادة استدعاء `GET /cart` للتأكد من الحالة الحالية للسلة، مع ملاحظة أنه بعد تنفيذ `POST /orders/checkout/confirm` يتم تفريغ العناصر وتحويل حالة السلة إلى `converted` وربطها بالطلب عبر `convertedToOrderId`.
