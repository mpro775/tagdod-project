...
2. [التحقق من وجود رقم الهاتف](#2-التحقق-من-وجود-رقم-الهاتف)
3. [التحقق من OTP وتسجيل الدخول](#3-التحقق-من-otp-وتسجيل-الدخول)
4. [تعيين كلمة المرور](#4-تعيين-كلمة-المرور)
5. [نسيت كلمة المرور](#5-نسيت-كلمة-المرور)
6. [إعادة تعيين كلمة المرور](#6-إعادة-تعيين-كلمة-المرور)
7. [الحصول على بيانات المستخدم](#7-الحصول-على-بيانات-المستخدم)
8. [تحديث بيانات المستخدم](#8-تحديث-بيانات-المستخدم)
9. [تحديث العملة المفضلة](#9-تحديث-العملة-المفضلة)
10. [حذف الحساب](#10-حذف-الحساب)
11. [تسجيل الدخول بكلمة المرور (User)](#11-تسجيل-الدخول-بكلمة-المرور-user)
12. [إنشاء حساب جديد بكلمة المرور](#12-إنشاء-حساب-جديد-بكلمة-المرور)
13. [Models في Flutter](#models-في-flutter)
...
1. ✅ يتم البحث عن المستخدم في قاعدة البيانات بالرقم كما هو (`777123456`)
2. ✅ إذا لم يُوجد، يتم البحث بالرقم المحول (`+967777123456`)
3. ✅ يتم إرجاع `exists: true/false` مع حالة الحساب (إن وجد)
...
if (checkResult.exists) {
  // الرقم موجود - إما تسجيل دخول أو استعادة كلمة مرور
  if (checkResult.status == 'ACTIVE') {
    navigateToLogin();
  } else if (checkResult.status == 'PENDING') {
    showDialog('الحساب في انتظار التحقق من OTP');
  } else if (checkResult.status == 'SUSPENDED') {
    showDialog('الحساب موقوف');
  }
} else {
  // الرقم غير موجود - يمكن التسجيل
  navigateToSignup();
}
```
...
if (checkResult.exists) {
  // الرقم موجود - إرسال OTP لتسجيل الدخول أو استعادة كلمة المرور
  await sendOtp(phone: phone, context: 'register');
} else {
  // الرقم غير موجود - إرسال OTP للتسجيل
  await sendOtp(phone: phone, context: 'register');
}
```
...
Future<CheckPhoneResponse> checkPhone({
  ...
  final apiResponse = ApiResponse<CheckPhoneResponse>.fromJson(
    response.data,
    (data) => CheckPhoneResponse.fromJson(data),
  );
...
1. ✅ يتم البحث عن المستخدم في قاعدة البيانات بالرقم كما هو (`775815074`)
2. ✅ إذا لم يُوجد، يتم البحث بالرقم المحول (`+967775815074`)
3. ✅ يتم تحويل الرقم إلى `+967775815074` فقط عند إرسال OTP عبر SMS
4. ✅ يتم حفظ OTP في Redis بالرقم المحول لمدة محددة (افتراضي: 5 دقائق)
...
Future<Map<String, dynamic>> forgotPassword(String phone) async {
  final response = await _dio.post(
    ...
    data: {'phone': phone},  // يُحفظ كما هو في قاعدة البيانات
  );
...
1. ✅ يتم تحويل الرقم إلى `+967775815074` للتحقق من OTP (لأن OTP يُحفظ بالرقم المحول)
2. ✅ يتم التحقق من OTP في Redis بالرقم المحول
3. ✅ يتم البحث عن المستخدم في قاعدة البيانات بالرقم كما هو (`775815074`) أولاً
4. ✅ إذا لم يُوجد، يتم البحث بالرقم المحول (`+967775815074`)
5. ✅ يتم تحديث كلمة المرور في قاعدة البيانات
6. ✅ يتم حذف OTP من Redis (لا يمكن استخدامه مرة أخرى)
...
Future<bool> resetPassword({
  ...
  final response = await _dio.post(
    '/auth/reset-password',
    data: {
      'phone': phone,  // يُحوّل إلى +967 فقط للتحقق من OTP
      'code': code,
...
## 11. تسجيل الدخول بكلمة المرور (User)
...
## 12. إنشاء حساب جديد بكلمة المرور
...
Future<SignupResponse> userSignup({
  ...
  final response = await _dio.post(
    '/auth/user-signup',
    data: {
      'phone': phone,  // يُحفظ كما هو في قاعدة البيانات
      'password': password,
      'firstName': firstName,
      'lastName': lastName,
      'gender': gender,
