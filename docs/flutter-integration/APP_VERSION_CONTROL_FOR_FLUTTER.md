...
- الـ Backend يخزّن **سياسة إصدار التطبيق** (أقل نسخة مسموح بها، نسخ محظورة، وضع صيانة، رابط التحديث).
- التطبيق (Flutter) يستدعي **endpoint عام** عند بدء التشغيل (وربما قبل عمليات مهمة) ويقرأ النسخة الحالية من `package_info_plus`.
- حسب النتيجة:
...
| `updateUrl`       | `string`   | رابط التحديث (متجر: Play Store / App Store) لزر "تحديث"                                                            |
...
1. **عند بدء التشغيل** (مثلاً بعد الـ splash أو في أول شاشة):
   ...
   - استدعاء `GET /api/v1/app/config` مع هيدر `X-App-Version: <version>`.
   - قراءة `data.canUse` و `data.shouldUpdate` و `data.maintenanceMode`.
...
- عند استلام **426** من أي استدعاء API:
  ...
  - استخدم `updateUrl` من آخر استجابة ناجحة لـ `/api/v1/app/config` إن وُجدت، أو احفظها عند أول نجاح.
...
| 3   | فحص الاستجابة عند **بدء التشغيل** (main أو أول شاشة بعد الـ splash) | حسب `canUse`, `shouldUpdate`, `maintenanceMode`                                                                                 |
| 4   | شاشة **تحديث إجباري**                                               | عند `shouldUpdate == true`: رسالة + زر يفتح `updateUrl`، بدون إمكانية المتابعة                                                  |
| 5   | شاشة **وضع الصيانة**                                                | عند `maintenanceMode == true`: رسالة فقط، بدون متابعة                                                                           |
| 6   | **(اختياري)** التعامل مع **426** من أي API                          | إظهار شاشة التحديث واستخدام `updateUrl` المحفوظ أو من آخر نجاح لـ app/config                                                    |
| 7   | **(اختياري)** إرسال **X-App-Version** في طلبات أخرى                 | يساعد الـ Backend إن فُعّل Guard على مسارات معينة                                                                               |
...
لا حاجة لـ `Authorization` لهذا الـ endpoint.
...
- **هيدر مُفضّل:** `X-App-Version: <نسخة التطبيق>`.
- **القرار:**
  - `maintenanceMode == true` → شاشة صيانة.
  - `shouldUpdate == true` → شاشة تحديث إجبارية + زر يفتح `updateUrl`.
  - `canUse == true` → متابعة عادية.
- **426:** إن وُجدت، عالجها كتحديث إجباري واستخدم `updateUrl`.

بعد تنفيذ النقاط أعلاه يكون تطبيق Flutter متكاملاً مع التحكم باصدار التطبيق من الـ Backend ولوحة الإدارة.
