...
2. [التكامل مع خدمة السلة](#2-التكامل-مع-خدمة-السلة)
...
## 2. التكامل مع خدمة السلة

تقوم خدمة السلة (`/cart`) باستدعاء `marketingService.validateCoupon` داخليًا عند تطبيق أي كوبون. أفضل مسار في واجهة Flutter يكون على الشكل التالي:

1. **تحقق سريع:** استخدم `/marketing/coupons/validate` لإظهار حالة الكوبون للمستخدم قبل التطبيق (صالح، منتهي، يتطلب حد أدنى...).
2. **تطبيق الكوبون:** استدعِ `POST /cart/coupon` مع `{ "couponCode": "<CODE>" }`. الاستجابة تعيد كائن `CartPreview` يحتوي على:
   - `appliedCoupons`: قائمة بكل الأكواد المطبقة بالترتيب.
   - `pricingSummary.couponDiscount`: إجمالي الخصم الناتج عن الكوبونات.
   - `pricingSummaryByCurrency`: نفس الحقول لكن لكل عملة مدعومة (USD/YER/SAR).
3. **إزالة الكوبون:** استدعِ `DELETE /cart/coupon` لمسح جميع الكوبونات (السلوك الحالي)، وستحصل على `CartPreview` محدث بدون الخصومات.

> **خاصية مهمة:** الباك-إند يدعم تكديس عدة كوبونات (إن سمح المنطق التجاري)، ويتم احتساب كل كوبون على المبلغ المتبقي بعد الخصومات السابقة.

### نموذج Flutter مبسط

```dart
Future<void> onApplyCoupon(String code) async {
  final preview = await cartApi.applyCoupon(code: code);
  cartState.updateFromPreview(preview);
}

Future<void> onRemoveCoupons() async {
  final preview = await cartApi.removeCoupons();
  cartState.updateFromPreview(preview);
}
```

> استخدم نماذج `CartPreview`، `CartPricingSummary`، وغيرها الموثقة في [خدمة السلة](./04-cart-service.md#8-models-في-flutter) لإعادة استخدام نفس البيانات في العرض.
