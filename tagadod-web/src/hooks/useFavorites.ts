...
function extractProductId(f: {
  ...
  if (p && typeof p === 'object') return (p as { _id?: string; id?: string })._id ?? (p as { _id?: string; id?: string }).id ?? ''
  ...
  const addMutation = useMutation({
    ...
    onSuccess: (_, productId) => {
      queryClient.setQueryData<PaginatedResponse<FavoriteItem>>(
        ['favorites'],
        (prev) => {
          const current = prev ?? { data: [] as FavoriteItem[], meta: { page: 1, limit: 100, total: 0, totalPages: 0 } }
          ...
          const meta = current.meta ?? { page: 1, limit: 100, total: 0, totalPages: 0 }
          return {
            ...current,
            data: [...data, { id: `opt-${productId}`, productId } as FavoriteItem],
      ...
      queryClient.setQueryData<PaginatedResponse<FavoriteItem>>(
        ...
        (prev) => {
          ...
          return {
            ...
            meta: prev.meta ? { ...prev.meta, total: Math.max(0, (prev.meta.total ?? 0) - 1) } : prev.meta,
          }
        },
      )
      queryClient.invalidateQueries({ queryKey: ['favorites'] })
