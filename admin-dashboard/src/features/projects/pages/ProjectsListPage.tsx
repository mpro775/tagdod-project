...
export const ProjectsListPage: React.FC = () => {
  ...
  const handleFiltersChange = (newFilters: ListProjectsParams) => {
  ...
  const handleSave = async (data: any) => {
    try {
      if (formDialog.mode === 'create') {
        ...
        await updateMutation.mutateAsync({ id: formDialog.project._id, data });
      }
      setFormDialog({ open: false, mode: 'create', project: null });
      refetch();
    } catch {}
  };
  ...
  const handleDeleteConfirm = () => {
    if (deleteDialog.project) {
      deleteMutation.mutate(deleteDialog.project._id, {
        onSuccess: () => {
  ...
  const columns: GridColDef[] = [
    {
      field: 'titleAr',
      headerName: t('table.columns.project'),
      width: 250,
      renderCell: (params) => (
        <Box>
    ...
    },
    {
      ...
      width: 130,
      renderCell: (params) => (params.row.type ? t(`types.${params.row.type}`) : ''),
    },
    {
      field: 'status',
      headerName: t('table.columns.status'),
      width: 130,
      renderCell: (params) => (params.row.status ? t(`status.${params.row.status}`) : ''),
    },
    {
      field: 'showOnLanding',
      headerName: t('table.columns.showOnLanding'),
      width: 120,
      renderCell: (params) => (params.row.showOnLanding ? 'نعم' : 'لا'),
    },
    {
      field: 'isFeatured',
      headerName: t('table.columns.isFeatured'),
      width: 100,
      renderCell: (params) => (params.row.isFeatured ? 'نعم' : 'لا'),
    },
    {
      field: 'isPublished',
      headerName: t('table.columns.isPublished'),
      width: 100,
      renderCell: (params) => (params.row.isPublished ? 'منشور' : 'مسودة'),
    },
  ...
  return (
    <PageShell fullHeight>
      ...
      <ProjectFormDialog
        ...
        onClose={() => setFormDialog({ open: false, mode: 'create', project: null })}
