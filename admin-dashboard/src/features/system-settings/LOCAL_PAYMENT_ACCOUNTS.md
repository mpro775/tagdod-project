...
### جدول البيانات
- يعتمد على المكوّن المشترك `DataTable` (مبني على `MUI DataGrid`).
- يدعم البحث، الفرز، والتحكم في الصفحات.
- الأعمدة الرئيسية:
  - اسم المزوّد + حالة التفعيل.
  - النوع (بنك/محفظة).
  - وضع الترقيم (مشترك / لكل عملة).
  - عرض سريع للحسابات (الرقم المشترك، العملات المدعومة، التخصيصات، حالة كل حساب).
  - أعمدة الإجراءات (تحرير / حذف).

### Dialog الإضافة/التعديل
- نموذج منظم مع جميع الحقول الضرورية وإدارة الحسابات الفرعية.
- التحقق من صحة الإدخالات يشمل:
  - اسم المزوّد.
  - رقم الحساب المشترك (في وضع المشترك).
  - اختيار عملة واحدة على الأقل (في وضع المشترك).
  - إضافة حساب واحد على الأقل (في وضع لكل عملة).
  - التأكد من وجود رقم لكل حساب فرعي.
- وجود Switch لتفعيل/تعطيل المزوّد بالكامل.
