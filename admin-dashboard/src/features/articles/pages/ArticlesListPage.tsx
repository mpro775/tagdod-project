    ...
    <Box sx={{ p: { xs: 2, sm: 3 } }}>
      <Box mb={3}>
        <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} alignItems={{ xs: 'flex-start', sm: 'center' }} justifyContent="space-between" gap={2}>
          <Box display="flex" alignItems="center" gap={2}><Article fontSize={isMobile ? 'medium' : 'large'} color="primary" /><Typography variant="h4">{t('pageTitle')}</Typography></Box>
      ...
      {isLoading ? (<Grid container spacing={2}>{[...Array(6)].map((_, i) => (<Grid size={{ xs: 12, sm: 6, md: 4 }} key={i}><Box sx={{ height: 280, borderRadius: 2, bgcolor: 'grey.100' }} /></Grid>))}</Grid>) : articles.length === 0 ? (<Box textAlign="center" py={8}><Article sx={{ fontSize: 64, color: 'text.disabled', mb: 2 }} /><Typography variant="h6" color="text.secondary">{t('messages.noArticles')}</Typography><Typography variant="body2" color="text.secondary">{t('messages.noArticlesDesc')}</Typography></Box>) : (<Grid container spacing={2}>{articles.map((a) => (<Grid size={{ xs: 12, sm: 6, md: 4 }} key={a._id}><ArticleCard article={a} onEdit={handleEdit} onDelete={handleDelete} onPublish={publishMutation.mutate} onArchive={archiveMutation.mutate} onToggleLanding={toggleLanding.mutate} onToggleFeatured={toggleFeatured.mutate} /></Grid>))}</Grid>)}
    </Box>
  );
};
