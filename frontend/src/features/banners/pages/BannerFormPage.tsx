...
import { z } from 'zod';
import { Box, Paper, Typography, Button, Grid, Divider, CircularProgress, FormControlLabel, Switch } from '@mui/material';
...
export const BannerFormPage: React.FC = () => {
  ...
  return (
    <Box>
      <Paper sx={{ p: 3 }}>
        <Typography variant="h5" fontWeight="bold" gutterBottom>{isEditMode ? 'تعديل البانر' : 'إضافة بانر جديد'}</Typography>
        <Divider sx={{ my: 3 }} />
        <FormProvider {...methods}>
          <form onSubmit={methods.handleSubmit(onSubmit)}>
            <Grid container spacing={3}>
              <Grid size={{ xs: 12 }}><FormInput name="title" label="العنوان *" /></Grid>
              <Grid size={{ xs: 12 }}><FormInput name="description" label="الوصف" multiline rows={2} /></Grid>
              <Grid size={{ xs: 12 }}><FormInput name="image" label="رابط الصورة *" /></Grid>
              <Grid size={{ xs: 12, md: 4 }}>
                <FormSelect name="type" label="النوع" options={[
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <FormSelect name="location" label="الموقع" options={[
              ...
              <Grid size={{ xs: 12, md: 4 }}><FormInput name="sortOrder" label="الترتيب" type="number" /></Grid>
              <Grid size={{ xs: 12, md: 6 }}><FormInput name="linkUrl" label="رابط الانتقال" /></Grid>
              <Grid size={{ xs: 12, md: 6 }}><FormInput name="linkText" label="نص الزر" /></Grid>
              <Grid size={{ xs: 12 }}>
              ...
              <Grid size={{ xs: 12 }}>
                ...
                <Box display="flex" gap={2}>
                  ...
                  <Button variant="outlined" startIcon={<Cancel />} onClick={() => navigate('/banners')}>إلغاء</Button>
                </Box>
              </Grid>
            </Grid>
          </form>
